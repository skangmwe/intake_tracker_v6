...
- **A configurable lifecycle** (Intake → Triage → Execution → Validation → Delivery → Stabilization → Closure, seeded AI-side only) with approval gates whose approver sets freeze at gate-open.
...
- **Request** — §17 (System/identity, Lifecycle/status, Intake, Value mapping, Solution details, Triage/classification, Execution, Delivery/outcome, Stabilization & closure value)
- **Feature Catalog** — §18
...
| **AI Solutions Analyst** | pick up escalated and AI-direct requests, work them through Intake → Closure, and see my load at a glance on the Home | I know what's mine, what needs me, and what changed since I last looked |
...
| Record detail | In-app detail page: header + sticky lifecycle stepper + **config-driven tab bar** (base: Status / Intake / Activity / Watchers & alerts; relationship-driven: Tasks & gates / Attachments) + Relationships side panel | Screen |
| Bridge provenance (escalated records) | **Inline** treatment (not a standalone panel): "Escalated · [origin]" header status pill + slim mirror note atop the Intake tab + pale-gold "⇄ Crossed · locked on PG" markers on crossing fields | Screen |
...
The build spec is itself the "good output" reference — 1,052 lines of executable design — and the prototype (`artifacts/docs/design/project/AI Solutions Tracker.dc.html`) is the visual truth for the prototyped screens (as of the 2026-07-16 v2 reconciliation, 22 of 43 screens are prototyped — the six core screens plus the Feature Catalog, Toolkit, and the full Workspace and Platform settings surfaces; see the blueprint's master table). In addition:
...
| 2026-07-16 | `/design-code-handoff` | Section 5 (Expected Output; Good Output) | Updated alongside the **v2 prototype reconciliation**. Corrected two stale Expected-Output rows to match the approved prototype: record detail is now a **config-driven tab bar** (base Status / Intake / Activity / Watchers & alerts; Tasks & gates and Attachments are relationship-driven tabs) with a Relationships side panel — not a fixed "Fields / Tasks & gates / Activity" set; and escalated-record bridge provenance is the **inline** treatment (status pill + mirror note + crossed-field markers), not a standalone three-part panel. Noted the prototype now covers **22 of 43 screens** (adds Feature Catalog, Toolkit, and the full Workspace + Platform settings surfaces). The v2 prototype also elaborated already-in-scope admin capabilities into rendered surfaces and codified several design-level concepts within existing intent — object-level **Relationships** + a **Link-to-record** field type (extends the typed-links model of §5 and the field-schema admin of §7), a record **Status/hold** model (In progress / On hold / Abandoned; hold pauses tasks + gates), **system-provisioned fields**, **multiple lifecycles** selected at intake, a **multi-dashboard composer** (the no-code builder pulled into R1 Phase 2 per the 2026-07-07 re-phasing), and the **Toolkit** (also R1 Phase 2). No change to tier, sensitivity, constraints, or the build spec's floors. Full detail in `full-design-blueprint.md` change log (2026-07-16 row). |
