...
9. **This is a multi-release build.** Do not build Release 2 capabilities into Release 1. In particular: no Toolkit build, no no-code dashboard builder, no request templates admin UI, no REST API + webhooks, no time-based triggers, no AI-assist, no DMS connector, no email delivery in Release 1. The Announcement object, however, ships in Phase 1.
