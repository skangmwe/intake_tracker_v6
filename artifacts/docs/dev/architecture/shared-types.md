...
| [`notifications.ts`](../../../../shared/types/notifications.ts) | `EventEnvelope`, `EventType`, `NotificationDto`, `NotificationCategory`, `NotificationQuery` + `UnreadCountDto` (bell feed + badge, slice 12), `SearchHitDto`, `SearchResultDto`, `SavedViewDto`, `SavedViewUpsertRequest`, `SavedDashboardDto`, `DashboardWidgetDto`, `WidgetType`. *(The Home types moved to `home.ts` when slice 22 built S1.)* |
