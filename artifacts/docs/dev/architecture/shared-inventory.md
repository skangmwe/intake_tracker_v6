...
## Slice 27 (Multiple lifecycles + Lifecycle picker at intake) — implemented

- **Shared type `/shared/types/gates.ts`** — new `LifecycleSummaryDto` `{ id, name, isDefault }` — the lightweight lifecycle list for the S3 intake picker and the S31 dropdown. `LifecycleDto` unchanged (still carries `requestType`, now deprecated as a user concept).
- **Shared type `/shared/types/requests.ts`** — `RequestCreateRequest` gains an optional `lifecycleId?: LifecycleId` (additive). When omitted the server uses the workspace default; preferred over the legacy `fields.requestType` string.
- **API `Modules/Lifecycle`** — new `GET /workspaces/{id}/lifecycles` (Viewer+) → `LifecycleSummaryDto[]`, backed by the **existing** `usp_GetWorkspaceLifecycles` (no new proc). `LifecycleService.GetSummariesAsync` + pure `LifecycleService.MapSummaries` (unit-testable; orders by sortOrder→name). The granular create/patch/set-default endpoints the v2 addendum sketched were **not** built — the slice-4 full-config `PATCH /workspaces/{id}/lifecycle` reconcile already covers them and the S31 dropdown reuses it.
- **API `Modules/Requests/RequestsService.ResolveLifecycle`** — new pure static helper: `(lifecycles, explicitLifecycleId?, requestType?) → LifecycleRow`, precedence explicit id → request-type string → default → first. Workspace-scoped list in, so a foreign id can never bind cross-workspace. Unit-testable. `RequestCreateRequest` (API DTO) gains `Guid? LifecycleId`.
- **Web `features/lifecycle/api.fetchWorkspaceLifecycles`** + **`useLifecycle.useWorkspaceLifecycles`** (+ `workspaceLifecyclesQueryKey`) — new thin fetch + query hook for the summary list; a separate cache key from the full config so the picker doesn't pull stages/gates it doesn't need. Consumers: the S3 `IntakeFormPage`.
- **Web `features/lifecycle/components/LifecyclesBar`** — reworked from a chip bar to the prototype **`<select>` dropdown** selector (fidelity fix). The request-type input was removed (the lifecycle `name` is the single label); `LifecycleDraft` dropped its `requestType` field, and `draftToRequest` mirrors `requestType` from `name` on save.
- **Web `features/requests/IntakeFormPage`** — the "Request type" select became the **"Lifecycle"** picker: options are lifecycle names, hidden when the workspace has one lifecycle, and the chosen `lifecycleId` is sent first-class on create.
- **Test fixture `web/src/test-utils.buildLifecycleSummary`** — new (`LifecycleSummaryDto`); consumed by the intake, hook, and (indirectly) e2e tests.
- **No DB migration.** The `Lifecycle` table was already multi-lifecycle-ready (slice 4 `IsDefault` + filtered unique index; slice 5 `Requests.LifecycleId` FK). Lifecycle immutability for a record's life is already enforced by `usp_SetRequestStage` reading `LifecycleId` off the record.

## Slice 28 (Multi-dashboard composer — S6 upgrade) — implemented

- **Shared type `/shared/types/dashboards.ts`** — new composed vocabulary (`ComposedWidgetMetric`, `ComposedWidgetDimension`, `WidgetWidth`); `DashboardWidgetConfig` gains composed fields (`composedMetric?`, `groupByDimension?`, `rowLimit?`, `width?`, `sortOrder?`, `depts?`, `stages?`) and `metric` becomes optional; `WidgetComposeRequest` refined to `depts?`/`stages?` arrays + required `title`; `DashboardComposeRequest`, `DashboardPatchRequest.{visibility,widgets}`; `SavedDashboardDto`/`DashboardListItemDto` gain `isSeeded?`/`visibility?`/`layoutMode?` and nullable `slug`. All additive — existing consumers compile unchanged.
- **Shared module `web/src/shared/dashboards/widgetTypeCatalog.ts`** — the single source of truth for the composer palette (`COMPOSER_WIDGET_TYPES`/`COMPOSER_METRICS`/`COMPOSER_DIMENSIONS`) + `widgetNeeds{Metric,Dimension,RowLimit}` predicates. Consumers: `WidgetComposerSheet`, `composerModel`.
- **API `Modules/Dashboards`** — `ResolveComposedAsync` (generic composed resolver, dispatches on widget type) alongside the fixed resolvers; `DashboardsService` create + widget CRUD (C# `WidgetsJson` manipulation → `usp_UpdateDashboard`) + seeded/author guards; controller endpoints + boundary validators. DB: `usp_CreateDashboard` + `usp_GetDashboardComposedKpi`/`…Breakdown`/`…Grid`; extended `usp_GetDashboardById`/`usp_ListDashboards`/`usp_UpdateDashboard`/`usp_ProvisionWorkspace`.
- **Web `features/dashboards`** — `useComposerScopeOptions` (dept options from the `deptPgClient` field via `useWorkspaceFields`; stages from the default lifecycle via `useLifecycleConfig`), `composerModel` (draft ↔ wire + `scopeLabel`), composer mutation hooks. Components: `DashboardSwitcher`, `NewDashboardSheet`, `WidgetComposerSheet`, `ComposerSheet` (reusable side-sheet chrome), `SegmentedToggle`, `ComposedDashboardSurface` — layered around the existing `DashboardSurface`/`WidgetRenderer`. `DashboardPage` reworked to host the switcher and route composed vs fixed.
