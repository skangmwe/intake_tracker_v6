// Data-driven form logic shared by every object whose form renders from a workspace field
// schema (FieldDefinitionDto[]) — Requests and custom-object records. Pure + unit-testable,
// no React. Fields are grouped by section and ordered by sortOrder, with the condition
// engine's Show/Hide/Require rules evaluated client-side so reveals happen live (the server
// re-validates on submit). Extracted from features/requests/requestForm.ts (its Request-specific
// pieces — INTAKE_CREATE_SECTIONS, computePriorityScore — stay there and re-export these).
...
/** Group visible (non-retired) fields by section, each ordered by sortOrder. */
...
/** Keep only the sections named in `sections`, in that order. */
...
/**
 * Validate a data-driven form on submit. Returns a field-key → message map (empty = valid).
 * Enforces required fields (base + conditional) and numeric min/max bounds where the schema sets them.
 */
