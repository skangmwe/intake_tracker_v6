...
// from api-testing-guidelines.md: happy path, permanent failure (no retry, no
// swallow), cancellation. Dependencies are mocked (no database, no Service Bus).
...
public sealed class EventSpineTests
{
    ...
    [Fact]
    public async Task EmitAsync_HappyPath_WritesAuditThenPublishes()
    {
        // Arrange
        ...
        var sut = new EventSpine(auditWriter.Object, publisher.Object);
        ...
        Assert.Equal(new[] { "audit", "publish" }, callOrder);
    }
    ...
    {
        // Arrange
        var sut = new EventSpine(Mock.Of<IAuditWriter>(), Mock.Of<IServiceBusPublisher>());
