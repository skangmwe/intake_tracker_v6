...
{
    // The five named built-ins, or a custom-object slug (ObjectDefinition.ObjectKey, generated by
    // usp_UpsertObjectDefinition, NVARCHAR(64)). No enumerating [RegularExpression] here — an earlier
    // version of this fix widened the built-ins-only regex to also admit a "clean" slug shape
    // ([a-z0-9][a-z0-9-]{0,63}), mirroring SavedViewUpsertRequest.ObjectType (Slice A / Task A6, see
    // SavedViewDtos.cs) — but that still rejects any slug the generator can actually produce outside
    // that shape: usp_UpsertObjectDefinition.sql's REPLACE chain only strips space/tab/CR/LF/_/&,
    // never apostrophes or other punctuation/non-ASCII, so e.g. "O'Brien Vendors" yields the ObjectKey
    // "o'brien-vendors" — which a charset regex would still 400 at model binding, before the
    // controller action, and therefore before FieldSchemaService's own existence check, ever runs.
    // We do not own the slug generator (shipped Slice-1 code) and no regex can safely track its full
    // charset, so binding does not attempt to enumerate valid object types at all: [Required] +
    // [MaxLength(64)] (matching the ObjectKey column width) is the full DTO-level contract, and the
    // service layer (FieldsController.ResolveObjectTypeAsync for the workspace path;
    // FieldSchemaService.UpsertGlobalObjectFieldAsync/RetireGlobalObjectFieldAsync for the platform
    // path) remains the sole authority on whether a value actually resolves to a real object — an
    // unresolvable value still 404s, and the value only ever reaches parameterized SQL. NOTE:
    // SavedViewUpsertRequest.ObjectType has the identical latent issue (same charset-regex approach,
    // same generator) and is a known sibling for a future slice — intentionally left unchanged here,
