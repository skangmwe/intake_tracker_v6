...
{
    // The five named built-ins, or a custom-object slug (lowercase, matching
    // ObjectDefinition.ObjectKey — generated by usp_UpsertObjectDefinition, NVARCHAR(64)) — mirrors
    // the same widened-regex fix already applied to SavedViewUpsertRequest.ObjectType (Slice A / Task
    // A6, see SavedViewDtos.cs) for the identical problem: the original built-ins-only allowlist
    // rejected every custom-object slug at model binding (400) before the controller action — and
    // therefore before FieldSchemaService — ever ran, on both the workspace field path
    // (FieldsController.ResolveObjectTypeAsync) and this Global-object field path
    // (FieldSchemaService.UpsertGlobalObjectFieldAsync/RetireGlobalObjectFieldAsync). Slug validity
    // (does it resolve to a real object) is enforced there, not by this regex, which only bounds the
    // shape/length; an unresolvable value still 404s.
    [Required]
    [RegularExpression("^(Request|Task|Feature|ToolkitItem|Attachment|[a-z0-9][a-z0-9-]{0,63})$")]
